...
void EventHandler::cmdQuit() {
	...
	for (std::map<std::string, Channel>::iterator channelIter = _channelList.begin(); channelIter != _channelList.end(); channelIter++)	{
	...
	for(size_t i = 0; i < channel.size(); i++) {
		if (channel[i] == ',' && channel[start] != '#') {
			...
			if (iter == _channelList.end()) {
				...
				_sendMsg = ":" + tmpfd.getNick() + "!" + tmpfd.getUser() + "@" + tmpfd.getHostname() + " " + _parse.begin()->second + " #" + channelName + "\r\n";
				sendMessage(_sendMsg);
				if (iter->second.getOper() == tmpfd.getFd())
					iter->second.setOper(-1);
	...
	for(size_t i = 0; i < channel.size(); i++) {
		if (channel[i] == ',' && channel[start] != '#') {
			...
			if (iter == _channelList.end()) {
				...
				_sendMsg = RPL_ENDOFNAMES + tmpfd.getNick() + " #" + channelName + " :End of NAMES list\r\n";
				sendMessage(_sendMsg);
				// :irc.example.com 353 me @ #test :@user1 +user2 user3 //RPL_NAMREPLY
				// :irc.example.com 366 me #test :End of /NAMES list. //RPL_ENDOFNAMES
			}
		}
	}
	return;
...
void EventHandler::cmdPrivmsg() {
	...
	if (target[0] == '#') {
		...
		if (getFd(target) == -1) {
			...
			//send privmsg
			// _sendMsg = ":buschat.net " + target + " :" + msg + "\r\n";
			_sendMsg = ":" + from.getNick() + "!" + from.getUser() + "@" + from.getHostname() + " PRIVMSG " + target + " :" + msg + "\r\n";
			sendMessage(_sendMsg);
			if (msg[0] == '@') {
				_fdTo.insert(_fdFrom);
